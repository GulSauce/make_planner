...
<%
    ...
    String str = "UPDATE MyInfo SET birth = ? WHERE id = ?";
...
<script>
    ...
    newNode.innerHTML = <%="\""+request.getParameter("birth")+"\""%>
    opener.document.getElementById("myInfo4").remove()
    newNode.id = "myInfo4"

    var temp = opener.document.getElementById("changeBirthBtn")

    temp.before(newNode)
    temp.value = "생일 변경"
    temp.setAttribute("onclick", "ChangeMyBirth(this)")

    opener.document.getElementById("cancelChangeBirthBtn").remove();
    alert( "생일 변경 완료")
    self.close();
</script>